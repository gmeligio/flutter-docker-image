...
The `gx_lint` job in `.github/workflows/build.yml` fails any pull request that introduces an unpinned `uses:` reference or a lock that disagrees with the workflows. If you forget step 3, the `gx-tidy` workflow will push a fixup commit on PRs from this repository (forks must run `gx tidy` locally).
