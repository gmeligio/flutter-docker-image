...
* \[x\] Installed Flutter SDK 3.13.5
...
```bash
# From Docker Hub
docker run --rm -it gmeligio/flutter-android:3.13.5 bash

# From GitHub Container Registry
docker run --rm -it ghcr.io/gmeligio/flutter-android:3.13.5 bash

# From Quay.io
docker run --rm -it quay.io/gmeligio/flutter-android:3.13.5 bash
```
...
jobs:
  build:
    ...
    container:
      image: ghcr.io/gmeligio/flutter-android:3.13.5
    steps:
      - name: Checkout
...
* Docker image: gmeligio/flutter-android:3.13.5
...
* `flutter_version <string>`: The version of Flutter to use when building. Example: 3.13.5
...
```bash
# Android
docker build --target android --build-arg flutter_version=3.13.5 fastlane_version=2.216.0 android_build_tools_version=30.0.3 --build-arg android_platform_versions="33" -t android-test .
```
