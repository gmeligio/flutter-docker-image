...
```bash
# From Docker Hub
docker run --rm -it gmeligio/flutter-android:3.13.4 bash

# From GitHub Container Registry
docker run --rm -it ghcr.io/gmeligio/flutter-android:3.13.4 bash

# From Quay.io
docker run --rm -it quay.io/gmeligio/flutter-android:3.13.4 bash
```
...
jobs:
  build:
    ...
    container:
      image: ghcr.io/gmeligio/flutter-android:3.13.4
    steps:
      - name: Checkout
...
* Docker image: gmeligio/flutter-android:3.13.4
...
* `flutter_version <string>`: The version of Flutter to use when building. Example: 3.13.4
