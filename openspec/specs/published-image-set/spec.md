# published-image-set Specification

## Purpose

Define the published image set as declared data rather than a list restated in
every workflow: what `config/images.json` holds, what the schema guards, and the
rule that each image matrix is derived from it. Covers per-concern membership —
an image may be published but not Scout-scanned, or built on pull requests but
push no handoff tag — and requires every exclusion to carry a reason. The
experience context is the CI engineer who expects an image this project
publishes to be treated as published everywhere, and the maintainer who needs to
