...
and SHALL cover **every published Linux image** (`flutter-android` and
`flutter-web`). For each image it SHALL render the per-image "Main tools" list
required by the `image-tool-inventory` capability, with every version sourced
from `config/version.json` — including Java (OpenJDK) from `android.java.version`.
The README SHALL include, per image, the registry badges and a Running Containers
...
platforms (GitHub Actions, GitLab CI, Gitea, Forgejo) and link to the `examples/`
directory, and SHALL include a generated table of contents.
