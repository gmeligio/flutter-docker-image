# linux-image-build-boundary Specification

## Purpose

Define the boundary that makes the Linux (`android.Dockerfile`) image build a single callable unit — a composite action under `.github/actions/` that owns the `docker/build-push-action` invocation and every build argument, used by all four Linux build legs (`build.yml` push path, `build.yml` fork path, `ci.yml`, `release.yml`). Covers where build arguments are declared, how unresolvable manifest values fail before a build starts, and the per-argument caching that the boundary preserves. The experience context is the CI engineer who trusts that the image validated on a pull request is built the same way as the image published on release, and who waits on a build that should rebuild only what actually changed.
