...
- **`check`** — runs on **all** PRs (incl. forks). `contents: read`, no token, default checkout (the merge ref is fine — nothing privileged runs). `pnpm run build` then `git diff --exit-code`. On a diff it fails with an explicit message naming the exact command to run (`pnpm --dir docs/src run build`) and to commit the result. This is the **required status check**; being token-free it is provably safe to run against untrusted fork code.
- **`generate`** — **fork-gated** (`if: github.event.pull_request.head.repo.full_name == github.repository`). `contents: write` scoped to the job, App token, checks out the PR head, builds, and commits+pushes the output onto the PR branch only if there is a diff. On same-repo PRs this auto-fixes the staleness; the push re-triggers `check`, which then passes. On fork PRs it is skipped, so the contributor only ever sees `check` fail.
