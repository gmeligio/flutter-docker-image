...
- **`update-docs.yml`** — change from a post-merge push to a **`pull_request`** workflow with two jobs over the committed docs output (`compile.js` turns `docs/src/*.mdx` into `readme.md`, `windows.md`, `contributing.md`, `license.md`):
  - **`check`** — runs on **all** PRs including forks, `contents: read`, no secret. Builds and `git diff --exit-code`s the output; if stale, fails with an explicit instruction to run the build and commit. This is the required status check and is provably safe on forks (no token, no write, no untrusted-code-with-secrets).
