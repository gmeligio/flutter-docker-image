...
- **`mise.toml`** — add a `[tasks.docs]` task (`mise run docs`) that regenerates the committed docs output, as the single source of truth for the docs build. It replaces the inlined `pnpm install && pnpm run build` in `update-docs.yml` (both jobs) and `update-version.yml`, and is the one command a contributor runs locally (from anywhere in the repo). mise already pins `node`/`pnpm`, so local and CI behavior are identical.
- **`update-docs.yml`** — change from a post-merge push to a **`pull_request`** workflow with two jobs over the committed docs output (`compile.js` turns `docs/src/*.mdx` into `readme.md`, `windows.md`, `contributing.md`, `license.md`):
  - **`check`** — runs on **all** PRs including forks, `contents: read`, no secret. Runs `mise run docs` then `git diff --exit-code`; if stale, fails with an explicit instruction to run `mise run docs` and commit. This is the required status check and is provably safe on forks (no token, no write, no untrusted-code-with-secrets).
