...
- **`update-docs.yml`** — change from a post-merge push to a **`pull_request`** workflow that regenerates the committed docs output (`compile.js` turns `docs/src/*.mdx` into `readme.md`, `windows.md`, `contributing.md`, `license.md`) and commits it back **onto the PR branch** when `docs/src/**` changed. The regenerated output is reviewed in the same PR that edited the source — no separate post-merge PR, no direct push to `main`. PRs touching `docs/src/**` come only from the maintainer and `renovate[bot]` (same-repo branches, never forks), so the App token is available and a plain commit+push to the PR head works. The feature branch is not covered by `required_signatures` (the ruleset targets `~DEFAULT_BRANCH` only); the final squash-merge commit on `main` is signed by GitHub.
