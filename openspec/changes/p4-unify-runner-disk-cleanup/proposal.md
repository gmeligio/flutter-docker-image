...
- Windows path swaps `Remove-Item -Recurse -Force` for `cmd /c rmdir /s /q` (faster on NTFS for large recursive trees), with `robocopy /MIR empty-dir target /MT:128 /NFL /NDL /NJH /NJS` as a fallback path for stubborn directories. Target: Windows cleanup completes in ≤ 4 minutes wall-clock.
...
- **Risk**: `rmdir /s /q` differs from `Remove-Item -Recurse -Force` on long-path / locked-file handling. Mitigation: keep a PowerShell fallback inside the action for paths that fail the cmd-based removal, and assert post-clean free space against a minimum threshold so a silent regression in what gets removed fails the job rather than being discovered as a downstream build OOM.
