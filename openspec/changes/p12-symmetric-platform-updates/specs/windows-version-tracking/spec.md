...
The composed `version.json` consumed by `update_docs_and_create_pr` SHALL be produced by the dedicated `compose_version_manifest` job, not by `update_docs_and_create_pr` itself. When `update_windows_version` did not produce a fragment, `compose_version_manifest` SHALL carry forward the `windows` block from the base branch unchanged.
...
- **AND** `validate_config_version` exits 0 against the composed artifact
