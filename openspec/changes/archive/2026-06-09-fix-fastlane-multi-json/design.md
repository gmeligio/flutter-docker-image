...
- **[Unrelated apt-pin drift blocks the build]** → a from-scratch build of `android.Dockerfile` hits stale Debian apt pins (#486), unrelated to fastlane. **Confirmed in CI on PR #491:** the build failed at the base apt install on `curl="8.14.1-2+deb13u2"` (mirror moved to `deb13u3`) *before* reaching the fastlane stage, and the registry buildcache no longer masked it, so `Test image (flutter-android)` was skipped. Resolution: this PR bumps `CURL_VERSION` to `deb13u3` to unblock CI and let the fastlane fix be validated end-to-end. This carries one slice of #486; the broader Renovate-pin drift fix remains separate.
