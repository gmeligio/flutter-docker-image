...
- **NEW — `.github/workflows/windows-image.yml`, a `workflow_call` reusable workflow** that owns the entire shared Windows build: `harden-runner` (first step) → checkout → `clean-runner-disk` → ensure-Docker-daemon → read version manifest → `docker/metadata-action` → (push-only) registry logins → `docker build windows.Dockerfile` with the five build-args (`flutter_version`, `git_version`, `vs_cmake_version`, `vs_win11sdk_build`, `vs_vctools_version`). Inputs: `target` (string) and `push` (boolean). It declares an explicit, least-privilege `secrets:` interface naming only the four credentials it uses (`DOCKER_HUB_USERNAME`, `DOCKER_HUB_TOKEN`, `QUAY_USERNAME`, `QUAY_ROBOT_TOKEN`, each `required: false`) — not `secrets: inherit`. The post-build tail branches on `push`: `docker run` the `--target test` image to execute the Pester suite (test), or push the tags to all three registries (release).
- **BREAKING — `windows.yml` becomes a thin caller.** Its `test-windows` job is replaced by a `uses: ./.github/workflows/windows-image.yml` call with `target: test`, `push: false`, forwarding **no secrets**. The inline build/cleanup/daemon/login/test steps are removed.
- **BREAKING — `release-windows` becomes a thin caller.** Replaced by a `uses: ./.github/workflows/windows-image.yml` call with `target: flutter`, `push: true`, forwarding all four Docker Hub + Quay secrets, and `permissions: { contents: read, packages: write }` on the caller job. This is what adds disk cleanup + daemon guard + harden-runner to the release path.
- **No login on the PR path.** All three registry logins are gated on `push` (true only on release). The PR test path builds from `mcr.microsoft.com`, installs Pester from PSGallery, and runs the locally-built image — it touches no registry, so a fork PR (no secrets) builds and tests with zero login attempts and no failure surface.
