...
- [x] 4.5 Confirm the fork-PR gate still skips the web Scout scan leg while the web build+test legs run on the local artifact (job-level `if:` preserved; consumers read build-image's shared handoff outputs and recompute only the per-image digest)
