...
- **Observing that pins actually resolve.** Deferred out of this change; see "Deferred verification" in `tasks.md`. Local verification covers URL construction only — the sandbox has no egress to `deb.debian.org`, so no lookup can succeed here. The confirming evidence is the first Renovate job log after merge.
