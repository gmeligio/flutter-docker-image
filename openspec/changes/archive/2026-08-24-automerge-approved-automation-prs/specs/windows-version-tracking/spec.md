...
**Reason**: Renamed. `update-version.yml` runs each weekday (`cron: '0 0 * * MON-FRI'`), so "monthly" misdescribes when the maintainer sees this PR. The behaviour is unchanged; only the cadence wording is corrected, here and in `flutter-version-update`.
