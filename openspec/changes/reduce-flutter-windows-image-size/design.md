...
**D1 — Swap the workload for explicit components.** Replace `--add Microsoft.VisualStudio.Workload.VCTools` with `--add Microsoft.VisualStudio.Component.VC.Tools.x86.x64`. The Windows 11 SDK and CMake components are already explicit `--add` lines and stay. `VC.Tools.x86.x64` is the MSVC x64/x86 build toolset (compiler, linker, libs) that MSBuild + Flutter consume; the workload's extra recommended components (ATL, MFC, additional SDKs, spectre-mitigated libs, etc.) are not needed for `flutter build windows`.

**D2 — The `vcTools.version` manifest field now tracks the component, not the workload.** `config/version.json`'s `windows.vsBuildTools.vcTools.version` keeps its key and CUE shape (`#SemverQuad`), but its meaning changes to the `VC.Tools.x86.x64` component version. Three consumers must move in lockstep: `update-version.yml:172` (jq selector `Workload.VCTools` → `Component.VC.Tools.x86.x64`), the Pester assertion (`Windows.Tests.ps1:96`), and the Dockerfile `--add`. The build-arg name `vs_vctools_version` / env `VS_VCTOOLS_VERSION` is unchanged to avoid churn in the workflow plumbing.
