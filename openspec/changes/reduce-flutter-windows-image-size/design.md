...
**D1 — Swap the broad VCTools workload for the narrower NativeDesktop workload.** Replace `--add Microsoft.VisualStudio.Workload.VCTools` with `--add Microsoft.VisualStudio.Workload.NativeDesktop`. The Windows 11 SDK and CMake components stay explicit. **Correction (learned from CI on PR #518):** the first attempt used the bare `Microsoft.VisualStudio.Component.VC.Tools.x86.x64` component, which failed `flutter build windows` with "Unable to find suitable Visual Studio toolchain" — Flutter's `vswhere` query requires a recognized C++ *desktop* toolchain (the `NativeDesktop` component group), which the raw VC.Tools component does not register. `Workload.NativeDesktop` (the documented Flutter requirement) is still materially narrower than the original `Workload.VCTools`, which dependency-pulls 20 components including v140/v141 toolsets, Clang, ATL/MFC, ASAN, vcpkg, and a second Windows SDK — the bloat this change sheds. `NativeDesktop` is installed WITHOUT `--includeRecommended` so only its required core lands.

**D2 — The `vcTools.version` manifest field now tracks the NativeDesktop workload.** `config/version.json`'s `windows.vsBuildTools.vcTools.version` keeps its key and CUE shape (`#SemverQuad`), but its meaning changes to the `Microsoft.VisualStudio.Workload.NativeDesktop` version. Three consumers must move in lockstep: `update-version.yml` (jq selector `Workload.VCTools` → `Workload.NativeDesktop`), the Pester assertion (`Windows.Tests.ps1:96`), and the Dockerfile `--add`. The build-arg name `vs_vctools_version` / env `VS_VCTOOLS_VERSION` is unchanged to avoid churn in the workflow plumbing.
