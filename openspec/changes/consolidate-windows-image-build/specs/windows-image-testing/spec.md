...
The `.github/workflows/windows.yml` workflow SHALL run on every `pull_request` event and SHALL verify the Windows image through a caller job that delegates to the reusable `.github/workflows/windows-image.yml` workflow (`uses:`) with `target: test`, `push: false`, `can-login` set to whether the PR head is in this repository, and `secrets: inherit`. The reusable workflow SHALL build `windows.Dockerfile` with `--target test` and run the Pester suite at `test/windows/Windows.Tests.ps1` inside that image. The PR check SHALL fail if the image build fails, if any Pester test fails, or if Pester exits non-zero. `windows.yml` SHALL NOT build `windows.Dockerfile` with its own inline steps.
