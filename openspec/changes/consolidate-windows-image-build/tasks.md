...
- [ ] 1.8 Logins, all gated `if: ${{ inputs.push }}`: Docker Hub (`secrets.DOCKER_HUB_*`), GHCR (`github.token`), Quay (`secrets.QUAY_*`). All `docker/login-action`, SHA-pinned. No login runs when `push` is false.
...
- [ ] 1.10 Post-build tail: when `inputs.push` is true, `foreach ($tag in $tags) { docker push $tag; check $LASTEXITCODE }`; when false, `docker run --rm <first metadata tag>` to execute the Pester suite — no `--env`/`-v`/args (the `test` image bakes in `config/version.json` + test files and overrides ENTRYPOINT/CMD to `RunPester.ps1`). Ensure the `docker run` exit code propagates (it's the last statement, or `if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }`) so a failing Pester test reddens the job.
