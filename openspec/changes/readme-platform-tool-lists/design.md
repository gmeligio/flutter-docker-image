...
- **Critical path:** the Java version the README shows equals the JDK the image ships. This is enforced structurally rather than by a new test — the value is **derived** from the running container (`script/java_version.sh`) and **displayed** from the same `version.json` field, so the two cannot diverge by construction.
...
- **Level:** no unit tests apply (config + docs + a CI step); integration is the schema gate plus the docs-sync check. An optional `test/android.yml` assertion that `java -version`'s major equals `android.java.version` is deferred (it would require extending the `config/android.cue` test-templating to inject the Java field).
