...
- [x] 4.3 Add a `test/android.yml` assertion that the built image's `java -version` major equals `android.java.version`: new "Java is pinned" commandTest (index 3), templated from a new `android_java_version` tag in `config/android.cue`, fed by `script/update_test.sh`. Verified: regeneration is idempotent (no drift); flipping `version.json` to `21` flips the expected output to `21` (test is manifest-driven); a built image whose JDK major differs from the manifest fails the structure test in `build.yml`
