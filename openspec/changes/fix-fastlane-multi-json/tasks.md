...
- [x] 2.4 If the cold build fails on unrelated Debian apt-pin drift (#486), note it and verify on a cold-build-clean base — that failure is out of scope for this change. DONE: cold build did hit `curl=8.14.1-2+deb13u2` (mirror moved to `deb13u3`); overridden via `--build-arg CURL_VERSION=...+deb13u3` at build time only. The committed Dockerfile keeps `deb13u2`; the curl bump belongs to #486.
