...
- Add `node = "lts"` to `mise.toml` (CUE pin already present).
...
- **REMOVE** `jaxxstorm/action-install-gh-release` and `actions/setup-node` from `.github/gx.toml` once no workflow references them, and prune their `.github/gx.lock` entries on the same PR (required by the `actions-version-tracking` spec's "mutually consistent on merge" invariant).

## Capabilities

### New Capabilities

- `ci-runtime-tool-versioning`: establishes `mise.toml` as the single source of truth for CI runtime tool versions (initially `cue` and `node`), consumed by every workflow via `jdx/mise-action@v4`. The capability covers: where versions live, how they reach `$PATH` in each job, and the invariant that no workflow may install these tools by any other mechanism. A CI engineer looking up "what version of cue does CI use?" SHALL find a single, authoritative answer in `mise.toml`.
...
- **Workflows**: `.github/workflows/ci.yml`, `.github/workflows/build.yml`, `.github/workflows/update_version.yml`, `.github/workflows/update_docs.yml`.
- **Manifests**: `mise.toml` (gains `node = "lts"`), `.github/gx.toml`, `.github/gx.lock`.
- **Runtime behavior**: `cue` and `node` continue to be on `$PATH` inside each job. Tool versions are now sourced from `mise.toml`.
