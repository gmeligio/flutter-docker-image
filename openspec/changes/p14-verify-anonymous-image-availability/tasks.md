...
- [ ] 1.1 Add `script/verify_published_image.sh` taking `<registry> <repository>
  <tag>` (e.g. `ghcr.io gmeligio/flutter-android 3.44.1`). `set -euo pipefail`.
- [ ] 1.2 Map registry host: `docker.io`/`registry-1.docker.io` →
  `registry-1.docker.io`; `ghcr.io` → `ghcr.io`; `quay.io` → `quay.io`.
- [ ] 1.3 Implement the standard token handshake: `HEAD /v2/<repo>/manifests/<tag>`
...
- [ ] 1.4 Exit `0` only on a final `200`; exit non-zero on `401`/`403`/any other
...
- [ ] 2.1 Add a `verify-published` job: `runs-on: ubuntu-24.04`,
...
- [ ] 2.2 The job performs **no** `docker/login-action` for any registry.
- [ ] 2.3 When `needs.release-android.result == 'success'`, run the script for
  `flutter-android` against `docker.io`, `ghcr.io`, and `quay.io` at
  `${{ github.ref_name }}` (the released version).
- [ ] 2.4 When `needs.release-windows.result == 'success'`, run the script for
  `flutter-windows` against the same three registries at `${{ github.ref_name }}`.
- [ ] 2.5 Any failing check fails the job (and thus the release run). Confirm no
...
- [ ] 4.1 **Before** the GHCR flip, run the script locally against the current
