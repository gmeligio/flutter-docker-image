...
Minimal Docker images for building Flutter apps in Continuous Integration (CI), for Android, Web, and Windows platforms, with the SDK and toolchain predownloaded so `flutter` runs without extra downloads. Images track the Flutter **stable** channel and the current version is **3.44.8**.

```bash
docker run --rm -it ghcr.io/gmeligio/flutter-android:3.44.8 flutter build apk
```

Each image is tagged with the Flutter version it ships (`:3.44.8`), there is no `latest` tag ([see more on the why](https://github.com/gmeligio/flutter-docker-image/blob/main/docs/faq.md#why-there-is-no-dynamic-tag-like-latest)). All tools running in the image have analytics disabled and opt-in with `ENABLE_ANALYTICS=true`, and a rootless `flutter:flutter` user.
...
| --- | --- | --- | --- |
| Docker Hub | [gmeligio/flutter-android:3.44.8](https://hub.docker.com/r/gmeligio/flutter-android) | [gmeligio/flutter-web:3.44.8](https://hub.docker.com/r/gmeligio/flutter-web) | [gmeligio/flutter-windows:3.44.8](https://hub.docker.com/r/gmeligio/flutter-windows) |
| GitHub Container Registry | [ghcr.io/gmeligio/flutter-android:3.44.8](https://github.com/gmeligio/flutter-docker-image/pkgs/container/flutter-android) | [ghcr.io/gmeligio/flutter-web:3.44.8](https://github.com/gmeligio/flutter-docker-image/pkgs/container/flutter-web) | [ghcr.io/gmeligio/flutter-windows:3.44.8](https://github.com/gmeligio/flutter-docker-image/pkgs/container/flutter-windows) |
| Quay | [quay.io/gmeligio/flutter-android:3.44.8](https://quay.io/repository/gmeligio/flutter-android) | [quay.io/gmeligio/flutter-web:3.44.8](https://quay.io/repository/gmeligio/flutter-web) | [quay.io/gmeligio/flutter-windows:3.44.8](https://quay.io/repository/gmeligio/flutter-windows) |
...
jobs:
  build:
    ...
    container:
      image: ghcr.io/gmeligio/flutter-android:3.44.8
    steps:
      - name: Checkout
...
For `flutter-web`, use the same workflow with `image: ghcr.io/gmeligio/flutter-web:3.44.8` and `run: flutter build web`. Windows containers cannot run under the Linux `container:` field, so `flutter-windows` runs on a `windows-2025` runner and invokes `docker` directly:
  ...
  build:
    ...
    steps:
      - name: Checkout
        uses: actions/checkout@v4
      - name: Build
        run: docker run --rm -v ${{ github.workspace }}:C:\app -w C:\app ghcr.io/gmeligio/flutter-windows:3.44.8 flutter build windows
```
